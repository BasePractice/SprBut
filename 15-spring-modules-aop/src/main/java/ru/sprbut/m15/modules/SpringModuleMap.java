 ...
 * <p>
 * Карта, оформленная как данные — чтобы утверждения о зависимостях между
 * модулями можно было проверить, а не запоминать.
 * <p>
 * Главное, что из неё следует: <b>Spring Core ни от чего не зависит</b>,
 * а всё остальное — прямо или косвенно от него. Именно поэтому «выучить Spring»
 * начинают с контейнера, а не с веба.
 */
public record SpringModuleMap(String name, Layer layer, String responsibility, List<String> dependsOn) {
    ...
    public static java.util.Set<String> transitiveDependencies(String moduleName) {
    ...
    private static void collect(String moduleName, java.util.Set<String> sink) {
        SpringModuleMap module = byName().get(moduleName);
