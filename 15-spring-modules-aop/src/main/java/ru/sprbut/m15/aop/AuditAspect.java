 ...
 * <p>
 * Аспект — это описание того, <b>что</b> сделать и <b>где</b>. «Где» задаётся
 * pointcut-выражением, «что» — телом advice-метода. Сам целевой класс при этом
 * не меняется ни одной строчкой: поведение живёт в обёртке.
 * <p>
 * Механически это ровно тот же {@code InvocationHandler} из модуля 04, только
...
public class AuditAspect {
    ...
    public Object around(ProceedingJoinPoint joinPoint) throws Throwable {
        log.add("around-start:" + joinPoint.getSignature().getName());
        try {
            Object result = joinPoint.proceed();
            log.add("around-end:" + joinPoint.getSignature().getName());
            return result;
        } catch (Throwable e) {
            log.add("around-error:" + joinPoint.getSignature().getName());
            throw e;
        }
    }
    ...
    public void afterThrowing(Throwable error) {
        log.add("afterThrowing:" + error.getClass().getSimpleName());
    }
