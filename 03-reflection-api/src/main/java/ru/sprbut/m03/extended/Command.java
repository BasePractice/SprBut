 ...
 * а метод контроллера принимает типизированные аргументы.
 */
public final class Command {
    ...
    public Invocation invocation() {
        int hash = this.text.indexOf('#');
        if (hash < 0) {
            throw new IllegalArgumentException(
                "Ожидался формат 'Класс#метод(...)', получено: " + this.text
            );
        }
        Spec target = new Spec(this.text.substring(0, hash).trim());
        Spec call = new Spec(this.text.substring(hash + 1).trim());
        Class<?> type = type(target.name());
        ChosenConstructor chosen = new ChosenConstructor(type, target.args().size());
        ChosenMethod method = new ChosenMethod(type, call.name(), call.args());
