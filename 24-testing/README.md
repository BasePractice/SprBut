...
|---|---|
| [`TestingApp`](src/main/java/ru/sprbut/m24/TestingApp.java) | Небольшое приложение, на котором показаны все виды тестов |
| [`ProductController`](src/main/java/ru/sprbut/m24/web/ProductController.java) | Веб-слой для среза `@WebMvcTest` |
| [`ProductRepository`](src/main/java/ru/sprbut/m24/domain/ProductRepository.java) | Интерфейс есть, реализации нет: JDK-прокси плюс разбор имён методов |
| [`CatalogService`](src/main/java/ru/sprbut/m24/domain/CatalogService.java) | В тесте контроллера подменяется, в своём — работает по-настоящему |
| [`SliceTests`](src/test/java/ru/sprbut/m24/SliceTests.java) | Полный контекст и три среза рядом, для сравнения |

## Расширенный пример

Расширенный пример этого модуля — сами [`SliceTests`](src/test/java/ru/sprbut/m24/SliceTests.java):
