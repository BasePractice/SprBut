 ...
 * <p>
 * Аудитор точек внедрения: разбирает класс рефлексией и выносит вердикт о том,
...
public final class InjectionAudit {
    ...
    public InjectionAudit(Class<?> type) {
        this(type, 5);
    }

    public InjectionAudit(Class<?> type, int limit) {
        this.type = type;
        this.limit = limit;
    }
    ...
    public Report report() {
        InjectionPoints points = new InjectionPoints(this.type);
        List<Style> styles = new ArrayList<>();
        List<String> dependencies = new ArrayList<>();
        List<String> warnings = new ArrayList<>();
        Constructor<?> injectable = points.constructor();
        ...
        List<Field> fields = points.fields();
        if (!fields.isEmpty()) {
        ...
        List<Method> setters = points.setters();
        if (!setters.isEmpty()) {
        ...
        boolean locator = ApplicationContextAware.class.isAssignableFrom(this.type);
        if (locator) {
