 ...
 * <p>
 * {@code RUNTIME} здесь обязателен: аспект читает аннотацию рефлексией уже
 * во время работы приложения, и с {@code CLASS} или {@code SOURCE} метка
 * до него просто не доживёт.
 * <p>
 * Сама по себе аннотация не делает ничего — это метаданные, а не поведение.
 * Работать её заставляет {@link AuditAspect}, ровно как {@code @Transactional}
 * работает благодаря прокси, а не собственной силе.
 */
@Documented
