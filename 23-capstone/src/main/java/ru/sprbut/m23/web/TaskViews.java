 ...
 * <p>
 * Реализации в исходниках нет — её пишет MapStruct во время компиляции,
 * а {@code componentModel = "spring"} помечает сгенерированный класс
 * как {@code @Component}, превращая его в обычный бин.
 * <p>
 * Разница с рефлексией принципиальна: сопоставление полей вычисляется один раз
 * компилятором, а в рантайме остаются прямые вызовы методов. Ошибка в имени поля
 * ломает сборку, а не прод.
 * <p>
 * Источники указаны явными выражениями, и это не прихоть: {@link Task} назван
 ...
 * тридцатилетней давности всё ещё определяет, что инструменты считают «свойством».
 */
