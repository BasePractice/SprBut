# Модуль 23 — Итоговое задание: SprBut Tracker
...
|---|---|
| 01–04 Reflection | [`ContextMap`](src/main/java/ru/sprbut/m23/extended/ContextMap.java) читает классы бинов и снимает прокси |
| 02 JavaBeans | [`TaskViews`](src/main/java/ru/sprbut/m23/web/TaskViews.java) — цена соглашения `getXxx` для инструментов |
| 05–06 Аннотации | [`Audited`](src/main/java/ru/sprbut/m23/audit/Audited.java): `RUNTIME`, `@Target(METHOD)`, элемент `value` |
| 07–09 APT | [`TaskViews`](src/main/java/ru/sprbut/m23/web/TaskViews.java) — реализации нет в исходниках, её пишет компилятор |
| 10 Lombok, MapStruct | [`TaskView`](src/main/java/ru/sprbut/m23/web/TaskView.java) на `@Value`/`@Builder` против `record` в [`NewTaskRequest`](src/main/java/ru/sprbut/m23/web/NewTaskRequest.java) |
| 11–12 IoC и DI | [`TaskService`](src/main/java/ru/sprbut/m23/service/TaskService.java): три зависимости через конструктор, включая `Clock` |
| 13–14 Bean и жизненный цикл | [`AuditTrail`](src/main/java/ru/sprbut/m23/audit/AuditTrail.java) — singleton, переживающий все запросы |
| 15 Spring AOP | [`AuditAspect`](src/main/java/ru/sprbut/m23/audit/AuditAspect.java) и ловушка `getMostSpecificMethod` |
| 16 Конфигурация | [`TrackerProperties`](src/main/java/ru/sprbut/m23/config/TrackerProperties.java), профиль `demo` |
| 17 Аннотации Spring | [`TaskController`](src/main/java/ru/sprbut/m23/web/TaskController.java), [`Failures`](src/main/java/ru/sprbut/m23/web/Failures.java) |
| 18 Запуск | [`TrackerRunner`](src/main/java/ru/sprbut/m23/startup/TrackerRunner.java) — `ApplicationRunner` |
| 19 Автоконфигурация | [`ClockAutoConfiguration`](src/main/java/ru/sprbut/m23/autoconfigure/ClockAutoConfiguration.java) + `AutoConfiguration.imports` |
...
| 22 AOT | [`TrackerHints`](src/main/java/ru/sprbut/m23/aot/TrackerHints.java) для DTO, которые собирает Jackson |

## Расширенный пример: карта контейнера

[`ContextMap`](src/main/java/ru/sprbut/m23/extended/ContextMap.java) — приложение,
